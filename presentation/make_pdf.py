...
def build_story(styles):
    ...
    story += section("3. Инновация: тектоническое расстояние", S)
    story.append(Paragraph(
        "Классические методы кластеризации используют евклидово расстояние между "
        "гипоцентрами. Однако сейсмические волны и статическое поле напряжений "
        "распространяются <i>вдоль литосферных структур</i>, а не по прямой линии "
        "сквозь континентальный щит.",
        S["body"]
    ))
    bullet_points = [
        ...
        "• Для каждой пары событий алгоритм <b>Дейкстры</b> находит кратчайший "
        "путь вдоль границ плит → тектоническое расстояние r_tect.",
