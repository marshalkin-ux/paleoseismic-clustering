...
the model does not fully explain; circum-Pacific concentration, possibly
regional aftershocks, but not global chains beyond model scope
...
\begin{keypoints}
  ...
  \item \textbf{Primary ETAS null} (H\&S 2003, Table~\ref{tab:etas_primary}):
        ...
        by model; circum-Pacific, not global chains.
...
\textbf{Limitation:} upstream $\eta_0$/cluster membership via \texttt{identify\_clusters()}
was \textbf{not} re-run at $b=0.911$ --- may affect GK/ZBZ declustering but not
...
ETAS literature ($n=1000$, H\&S 2003) & Local aftershock-only synthetics & $p_\mathrm{ETAS}\le 0.001$: $N_\mathrm{obs}=27 > \bar{N}\approx 15.4$; clustering not explained by ETAS; circum-Pacific; not global chains \\
...
Literature (H\&S 2003) & 0.008 & 0.08 & $\approx 15.4$ & $\le 0.001$ &
  ...
  explained by model; circum-Pacific, not global chains \\
\hline
\end{tabular}
\end{table}
...
expectation --- deviation from ETAS indicates clustering the model does not
fully explain; circum-Pacific concentration, possibly regional aftershocks,
but not global chains beyond model scope.
...
Deviation from ETAS indicates clustering the model does not fully explain;
spatial analysis shows concentration within the circum-Pacific belt, possibly
reflecting regional aftershock sequences, but not global chains beyond model
scope. Quantitative excess also reflects detector rules (mean GC $>1500$~km,
...
Literature H\&S 2003 (primary) & $\le 0.001$ & Exceeds (mean $\approx 15.4$) &
  Clustering not explained by ETAS; circum-Pacific; possible regional aftershocks; not global chains \\
