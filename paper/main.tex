...
CSV rows, including ${\sim}151$ NOAA records with $M < 6.5$ from an
$M \ge 6.0$ fetch; 2{,}041 modern, 2{,}179 early instrumental,
47 pre-1900).
...
(Section~\ref{sec:results}); full validation without ground-truth data
is not possible.
...
$\to$ Gardner--Knopoff declustering (mainshocks for NN forest)
$\to$ $\eta$ nearest-neighbor forest with tectonic distance
$\to$ sliding windows (1/2/5~yr) $\to$ series criteria ($N \ge 4$,
$\ge 3$ Flinn--Engdahl regions) $\to$ Monte~Carlo / ETAS / FDR validation.
Gardner--Knopoff removes ${\sim}24$ local aftershocks before series search.
Zaliapin--Ben-Zion declustering (100\% independent events) is a
\textbf{sensitivity check}, not a sequential filter: GK and ZBZ answer
different questions (local aftershocks vs.\ $\eta$ bimodality).
