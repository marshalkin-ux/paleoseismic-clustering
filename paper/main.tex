...
the $\eta$-metric (sensitivity analysis; primary spatial gate is GC $> 1500$~km).
...
Table~\ref{tab:gk_zbz} compares modern-window (1973--2026) counts;
series count $N = 27$ is unchanged under either method.

\begin{table}[htbp]
\centering
\caption{Gardner--Knopoff vs.\ Zaliapin--Ben-Zion declustering (modern window).}
\label{tab:gk_zbz}
\begin{tabular}{lrrl}
\hline
Method & Role & Mainshocks / 2041 & Removed \\
\hline
Gardner--Knopoff & Primary & 2{,}017 & 24 (98.8\%) \\
Zaliapin--Ben-Zion & Sensitivity & 2{,}040 & 1 (100.0\%) \\
\hline
\end{tabular}
\end{table}
...
in Table~\ref{tab:gk_zbz} come from \texttt{run\_declustering\_comparison.py}.
...
\paragraph{Global ETAS limitations and falsification value.}
Our catalog-calibrated ETAS generator is a \textbf{global exploratory} null,
not a regional standard model of the kind calibrated network-by-network in
\citet{Ogata1988, HelmstettterSornette2003}.
Long-range coupling is explicitly disabled (${\le}500$~km); the minimal MLE
(\texttt{scripts/calibrate\_etas.py}) does not implement full spatial Ogata
(1998) MLE.
Within these constraints, $p_\mathrm{ETAS}=1.0$ is a \textbf{meaningful
falsification}: the liberal detector finds no \emph{excess} multi-regional
structure beyond what local aftershock branching reproduces.
Negative results constrain over-interpretation of descriptive cluster lists
and complement positive-detection studies that lack an ETAS benchmark.
...
\item \textbf{Heuristic tectonic metric:} 98\% of pairs use 1.5$\times$ GC fallback;
      labeled sensitivity/failed hypothesis test; primary spatial gate is
      mean pairwise GC $> 1500$~km.
