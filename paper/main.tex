...
under \textbf{catalog-matched} ETAS calibration it finds mean \textbf{27.0} false
series, matching $N_\mathrm{obs}=27$ ($p_\mathrm{ETAS}=1.0$) --- a \textbf{coupled}
detector--calibration outcome, not sole proof of absent global structure.
Literature-parameter ETAS (H\&S 2003: $\mu=0.008$, $K=0.08$) yields mean
$\approx 15.4$ ($p_\mathrm{ETAS}\le 0.001$): $N_\mathrm{obs}=27$ \textbf{exceeds}
that local-clustering null, but this tests aftershock structure, not proven
teleseismic chains (see \S\ref{sec:etas-calib-limits}, Table~\ref{tab:dual_null}).
...
\begin{keypoints}
  ...
  \item Dual ETAS nulls (Table~\ref{tab:dual_null}): literature params
        $\Rightarrow$ mean $\approx 15.4$, $p_\mathrm{ETAS}\le 0.001$;
        catalog-calibrated $\Rightarrow$ mean $= 27.0$, $p_\mathrm{ETAS}=1.0$
        (detector--calibration coupling; not sole falsification).
...
\hline
Null model & $\mu$ & $K$ & Mean false series & $p_\mathrm{ETAS}$ & Interpretation \\
\hline
Literature (H\&S 2003) & 0.008 & 0.08 & $\approx 15.4$ & $\le 0.001$ &
  $N_\mathrm{obs}=27$ exceeds local-clustering null \\
Catalog-calibrated (WLS) & 0.103 & 0.495 & 27.0 & 1.0 &
  Indistinguishable from detector (coupled calibration) \\
\hline
\end{tabular}
\end{table}
...
Literature-parameter ETAS (Table~\ref{tab:dual_null}) yields mean $\approx 15.4$
and $p_\mathrm{ETAS}\le 0.001$: $N_\mathrm{obs}=27$ \textbf{exceeds} that null,
but this tests \textbf{local aftershock clustering} only, not proven teleseismic
chains.
The negative outcome stands on other grounds: no physical mechanism, failed
tectonic metric (98\% GC fallback), and liberal search space --- yet the ETAS
benchmark itself is \textbf{parameter-sensitive}.
...
\begin{enumerate}
  \item The permutation test rejects a global temporal Poisson null
        ...
        global series.
  \item The detector is \textbf{liberal}: catalog-matched ETAS yields mean $= 27.0$,
        $p_\mathrm{ETAS}=1.0$ (coupled calibration); literature ETAS yields mean
        $\approx 15.4$, $p_\mathrm{ETAS}\le 0.001$ (Table~\ref{tab:dual_null}) ---
        parameter-sensitive null; no physical mechanism or validated tectonic metric
        supports global-series claims (see \S\ref{sec:etas-calib-limits}).
