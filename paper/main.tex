...
catalog of 4{,}418 events ($M \ge 6.5$; 2{,}041 modern, 2{,}179 early
instrumental, 198 pre-1900).
...
After deduplication and quality filtering, the full catalog contains
4{,}418 events with $M \ge 6.5$ spanning 2150~BCE to 2026~CE:
2{,}041 modern events (1973--2026), 2{,}179 early instrumental (1900--1972),
and 198 pre-1900 historical events (fragmentary over $\sim$4{,}000~years).
For the modern window, USGS~ComCat provides 2{,}088 raw $M \ge 6.5$ records;
after \texttt{quality\_score}~$> 0.5$ filtering, 2{,}041 events remain
($\sim$47 excluded).  Gardner--Knopoff declustering removes $\sim$24 local
aftershocks during analysis (2{,}017 independent mainshocks, 98.8\%).
Each event receives a quality score in $[0.30, 0.95]$ based on epoch,
phase readings, and cross-catalog overlap \citep{Woessner2005}; analysis
is restricted to windows where $>90\%$ of events exceed
\texttt{quality\_score}~$= 0.5$.
...
Parameters are estimated by maximum likelihood from the declustered
catalog using the procedure of \citet{Ogata1998}; representative global
values are $\mu \approx 0.01$, $K \approx 0.08$, $\alpha \approx 1.0$,
$c \approx 0.01$~yr, $p \approx 1.1$ \citep{HelmstettterSornette2003}.

We generate 100 synthetic ETAS catalogs, each containing the same number
of events as the observed catalog and matching its magnitude--frequency
distribution, but with no spatial coupling beyond the local aftershock
kernel.
The global-series algorithm is then applied to each synthetic catalog,
yielding a null distribution for the number of detected series.
The ETAS $p$-value is defined as $p_\mathrm{ETAS} = P(N^\mathrm{ETAS}_\mathrm{series}
\ge N^\mathrm{obs}_\mathrm{series})$.
...
The full historical analysis covers 4{,}418 events ($M \ge 6.5$, BCE--2026,
including historical catalog) and identifies 47 global seismic series in three
...
Pre-1900 (historical)          &  198 &  5 & $0.46$    & $-0.74$ \\
\hline
...
(FPR, fraction of catalogs with $\ge 1$ global series) is 0.000.
The ETAS $p$-value is $p_\mathrm{ETAS} = 0.0000$ (empirical resolution
$1/101$).  Because none of the 100 ETAS catalogs produced a global series,
...
\begin{enumerate}
  \item Full historical analysis (4{,}418 events, BCE--2026) identifies
