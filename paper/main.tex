...
catalog of 4{,}418 records containing 4{,}267 events ($M \ge 6.5$; 2{,}041 modern,
2{,}179 early instrumental, 47 pre-1900).
...
Background seismicity is isolated using the \citet{ZaliapinBenZion2013}
adaptive declustering, which exploits the bimodal distribution of
nearest-neighbor distances to separate clustered from background events.
...
\begin{keypoints}
  \item A unified catalog of 4{,}418 events ($M \ge 6.5$, 2150~BCE--2026) yields
        47 global seismic series validated by a three-level statistical framework:
...
The file \texttt{unified\_catalog\_full.csv} contains 4{,}418 rows;
after $M \ge 6.5$ filtering, 4{,}267 events remain spanning 2150~BCE to
...
of the parent event.
Note: $b = 1.0$ follows the original \citet{Baiesi2004} formulation;
the catalog's empirical $b$-value is $0.911 \pm 0.018$ (used in the
Monte~Carlo test).
...
Implementation values in \texttt{ETASCatalogGenerator}
(\texttt{src/analysis/etas\_validation.py},
\texttt{scripts/run\_etas\_validation.py}) are $\mu = 0.008$~day$^{-1}$,
...
The parameter $c = 0.005$~day was chosen conservatively to accelerate
aftershock decay in the null model; even under this strict condition,
no false series emerged (FPR~=~0/100).
...
500~km.
The global-series algorithm (\texttt{global\_series}: $N \ge 4$,
...
$\ge 1$ spurious global series; results
(\texttt{results/etas\_validation.json}): mean false series
...
mainshocks; Zaliapin--Ben-Zion retains 100.0\% (2040/2041), confirming
that the catalog is dominated by independent events rather than
...
replicates, the observed multi-regional series cannot be explained by
standard aftershock branching alone.
Together, these results provide the first quantitative evidence that
global $M \ge 6.5$ catalogues contain multi-regional seismic episodes
that are neither random Poisson fluctuations nor local aftershock
artefacts.
\paragraph{Early instrumental period (1900--1972).}
Fifteen series in this window reach statistical significance
($p = 0.007$, $z = -2.43$), strengthening the overall conclusion that
clustering is not limited to modern catalog completeness.
However, most pre-1960 events have \texttt{quality\_score}~$< 0.7$,
and early instrumental incompleteness requires cautious interpretation
of individual episodes (including 1905--1910).
Previous null results \citep{Michael2011, ShearerStark2012} likely
reflect the use of Euclidean metrics and the absence of tectonic
connectivity constraints; replacing Euclidean distance with tectonic-path
distance, combined with ETAS and FDR validation, exposes a signal that
was previously masked by methodological noise.
Either a genuine long-range triggering mechanism operates at global
...
Three physical mechanisms can account for inter-event
...
\paragraph{Experiment 3.2 --- Tectonic-distance weight.}
The relative weight $\alpha$ controlling the tectonic-path penalty (see
Section~\ref{subsec:weights}) is varied $\pm 30\%$.  The Jaccard
similarity between the reference series set and the perturbed set exceeds
0.72 in all trials, indicating robust series membership.
...
\begin{enumerate}
  ...
  \item Given current evidence, two interpretive paths remain open:
        \textbf{(a)}~if detected series reflect genuine long-range coupling,
        probabilistic hazard models should incorporate elevated conditional
        probabilities in tectonically connected distant zones and explicit
        long-range kernels in ETAS extensions;
        \textbf{(b)}~even if individual episodes prove artifactual under
        refined constraints, the combined FDR~+~ETAS framework provides a
        reproducible, code-audited null-testing pipeline for future catalog
        updates.
  \item Global seismic series are not reproduced by ETAS models without
        long-range coupling, implicating stress-transfer mechanisms beyond
        local aftershock propagation.
  \item \textbf{Interpretive fork given current evidence:}
        \textbf{(a)}~If detected series reflect genuine long-range coupling,
        hazard models should incorporate elevated conditional probabilities
        in tectonically connected distant zones and explicit long-range
        kernels in ETAS extensions;
        \textbf{(b)}~even if individual episodes prove artifactual under
        refined constraints, the combined FDR + ETAS framework provides a
        reproducible, code-audited null-testing pipeline for future catalogs.
\end{enumerate}
...
Data and code are archived at \url{https://github.com/example/paleoseismic-clustering}
