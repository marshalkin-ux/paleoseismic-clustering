...
(1973--2026, 2{,}041 events), catalog-calibrated ETAS without long-range links
($>500$~km) reproduces $N_\mathrm{obs}=27$ detector episodes
(mean~$= 27.0$, $p_\mathrm{ETAS} = 1.0$, FPR~$= 1000/1000$, $n=1000$) ---
\textbf{no excess ``global'' structure beyond ETAS-like local clustering}.
...
27 modern); Benjamini--Hochberg FDR (45/47 at $q=0.05$) \textbf{does not}
correct the full search space.
...
\begin{keypoints}
  \item Catalog-calibrated ETAS: $N_\mathrm{obs}=27$, mean~$= 27.0$,
        $p_\mathrm{ETAS}=1.0$ --- \textbf{no evidence for excess global structure}
        beyond local ETAS-like clustering (null/falsification result).
  ...
  \item 47 detector candidates (45/47 FDR post-hoc) are indistinguishable from
        ETAS null plus liberal search artifacts; tectonic distance failed as a
        global metric (98\% GC fallback).
\end{keypoints}
...
\begin{enumerate}
  \item The permutation test rejects a global temporal Poisson null
        ...
        global series.
  \item Catalog-calibrated ETAS reproduces $N_\mathrm{obs}=27$
        (mean~$= 27.0$, $p_\mathrm{ETAS} = 1.0$, FPR~$= 1000/1000$) ---
        \textbf{no excess global structure} beyond ETAS-like local clustering;
        the multi-regional global-series hypothesis is \textbf{falsified}.
  \item 47 detector candidates (45/47 FDR post-hoc) are indistinguishable
        from ETAS null plus liberal search artifacts; tectonic distance is
