...
\title{Temporal clustering of global $M \ge 6.5$ earthquakes is consistent with ETAS: a hold-out validation}
...
\textbf{Catalog-calibrated temporal ETAS:} $p_\mathrm{ETAS}=1.0$ (consistent with the
in-sample temporal null; full numbers --- \S\ref{subsec:primary-counts}).
The title refers to \textbf{detection geometry} ($\eta$ + GC $> 1500$~km gates);
\textbf{inferential tests are temporal-only}.
Pre-1900 NOAA records --- Supplementary Material S3.
\end{abstract}
...
\begin{keypoints}
  ...
  \item \textbf{Primary ETAS null} (temporal MLE): $p_\mathrm{ETAS}=1.0$ --- see
        \S\ref{subsec:primary-counts}; spatial component not modeled.
...
Large earthquakes are not independent in time.
\citet{Michael2011} tested whether global $M \ge 7$ clustering in 1995--2011
exceeds Poisson rate fluctuations.
\citet{ShearerStark2012} tested whether global $M \ge 7$ and $M \ge 8$ rates
increased after the 2004 Sumatra earthquake.
Both found no anomalous global clustering beyond standard null models, but
neither tested multi-regional $\eta$-linkage at $M \ge 6.5$ with
catalog-calibrated ETAS validation.

\textbf{Two-phase design.} (1)~\textbf{Detection phase}---spatiotemporal
\emph{candidate search}: Baiesi--Paczuski $\eta$ nearest-neighbor forest plus
sliding-window detector with spatial gates (mean GC $> 1500$~km, $N \ge 4$,
$M \ge 6.5$).
(2)~\textbf{Validation phase}---\emph{temporal-only} catalog-calibrated ETAS MLE
plus hold-out split; this phase does \textbf{not} test whether geographically
dispersed candidates form a physically linked global series (spatial Ogata 1998
kernel---future work).

\textbf{Objective.} Temporal ETAS falsification plus a reproducible pipeline
for global $M \ge 6.5$ detector output (primary window 1973--2026).

\textbf{Scope.} Phase~(1) yields \textbf{detector candidates}; phase~(2) reports
\textbf{temporal excess} vs.\ catalog-calibrated ETAS only; spatial linkage is
\textbf{not tested}.
Bird (2003) tectonic distance was a \textbf{preliminary idea, not validated}
(Supplementary S1; excluded from the primary pipeline).
Conclusions are limited to temporal clustering in detector windows.

\textbf{Contribution.} This work provides a \textbf{reproducible} global
$M \ge 6.5$ pipeline (merged catalog, Baiesi--Paczuski detector,
catalog-calibrated temporal ETAS null) under explicit \textbf{falsification}
framing and \textbf{bounds of inference}: primary ETAS uses \textbf{in-sample}
calibration on 1973--2026 GK mainshocks with the \textbf{same} detector;
spatial long-range linkage is \textbf{not} tested.
The value is methodology plus honest null-result bounds---not a discovery claim.
We do \textbf{not} claim to have disproved global series as a physical phenomenon;
we bound what the implemented tests can and cannot establish.
...
The detector uses spatial gates (mean GC $> 1500$~km); inferential tests are
temporal-only (see \S\ref{sec:limitations}).
...
\hline
(a) Permutation (diagnostic) & Independent event times (fixed coordinates) & Temporal clustering & mean $\log_{10}\eta_\mathrm{NN}$; $n=10{,}000$ & \textbf{Diagnostic (Methods):} Poisson times rejected---expected with aftershocks \\
(b) ETAS calibrated (MLE) & $N_\mathrm{series} \le$ ETAS synthetic expectation & Excess series count & $n=1000$ synthetics & see \S\ref{subsec:primary-counts} \\
(c) Global-series hypothesis & No teleseismic multi-regional chains beyond local clustering & Such chains exist & --- & \textbf{Not tested (spatial ETAS open)} \\
(d) WLS negative control & Detector--calibration coupling artifact & --- & Supplementary S2 & \textbf{Not} primary null \\
\hline
\end{tabular}
\end{table}
...
Supplementary Material S3 (\texttt{paper/supplementary.md}).}}
...
(\texttt{results/sensitivity\_b0911\_full\_pipeline.json}):
full pipeline re-run at $b=1.0$ vs $b=0.911$ (GK $\to$
\texttt{identify\_clusters()} $\to$ \texttt{global\_series()}) yields
$N_\mathrm{series}=27$ both; Jaccard of series event sets $=1.0$.
\textbf{Equal $N=27$ does not prove candidate identity:}
\texttt{global\_series()} gates dominate; upstream cluster labels differ for
\textbf{8.2\%} of events (165/2017) when $b$ changes in $\eta$.
...
\noindent The Bird (2003) tectonic-path heuristic is \textbf{excluded from the
primary pipeline}; no synthetic benchmark against Bird geometry is included in
this work (see \texttt{paper/supplementary.md} \S S1).
...
\noindent\textbf{Stability of $N=27$ (Table~\ref{tab:gk_zbz}).}
At fixed gates ($\Delta t = 2$~yr, mean GC $> 1500$~km, $N \ge 4$) the same
event kernel survives merge \textbf{142 $\to$ 47 $\to$ 27} under GK, ZBZ, and
no declustering (Jaccard of series \textbf{event sets} vs GK baseline $= 1.0$),
but Jaccard of \textbf{series membership} vs baseline $= 0.32$ for ZBZ/none
(\texttt{results/series\_stability\_venn.json}).
This reflects \textbf{liberal detector gates}, not proof of physical robustness.
...
Benjamini--Hochberg & --- & Post-hoc on 47 merged candidates; window-level BH on 142 overlapping tests not a discovery procedure (\texttt{results/fdr\_windows.json}) \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{Detector gate (sole mandatory criteria):} $N \ge 4$,
        ...
        Mean pairwise GC exceeds a threshold on \textbf{averaged} pairs and is
        \textbf{weaker} than requiring every unordered pair to exceed 1500~km;
        we acknowledge this as a permissive spatial gate.
...
\textbf{In-sample note:} $p_\mathrm{ETAS}=1.0$ reflects calibration and
validation on the \textbf{same} modern window (1973--2026) with the
\textbf{same} detector---not independent out-of-sample proof of absence of
anomalies.
We report ``consistent with the \textbf{in-sample temporal null}'' rather
than ``proves no anomalies.''
ETAS is a \textbf{triggering} model fit on GK-\textbf{declustered} mainshocks;
declustering removes events the model would treat as offspring---an
acknowledged model mismatch.
Complementary hold-out validation (train 1973--2000, validate 2001--2026;
\texttt{scripts/calibrate\_etas\_holdout.py},
\texttt{results/etas\_holdout\_validation.json}) --- \S\ref{subsec:holdout-methods}.
WLS minimal calibration (\texttt{scripts/calibrate\_etas.py}) is a \textbf{negative control}
(Supplementary S2).
...
\subsection{Hold-out temporal ETAS validation}
\label{subsec:holdout-methods}

To complement the \textbf{in-sample} primary test (\S\ref{subsec:etas-calibration}),
we fit temporal MLE on GK mainshocks in \textbf{1973--2000} only
(\texttt{scripts/calibrate\_etas\_holdout.py}; $n_\mathrm{train}=1024$ mainshocks)
and validate $N_\mathrm{series}$ on the hold-out catalog \textbf{2001--2026}
($n_\mathrm{holdout}=1010$ events) with \textbf{train-calibrated parameters
applied without re-fitting}.
Detector unchanged: $\Delta t = 2$~yr, mean pairwise GC $> 1500$~km, $N \ge 4$.
We generate $n = 1000$ synthetic catalogs (seed~$= 42$;
\texttt{results/etas\_holdout\_validation.json}: $N_\mathrm{obs}=13$,
mean $= 13.0$, $p_\mathrm{ETAS}=1.0$).
This is a \textbf{partial out-of-time} check, \textbf{not} full spatial validation.
...
$p_\mathrm{ETAS} = 1.0$ --- $N_\mathrm{obs}=27$ is \textbf{consistent with the
in-sample temporal null}, not proof of absence of clustering in general.
...
WLS (Supplementary S2) & 0.103 & 0.495 & 27.0 & 1.0 &
...
\subsection{Primary counts (modern window, 1973--2026)}
\label{subsec:primary-counts}

Merged CSV: 4{,}418 rows $\to$ 4{,}267 unique $M \ge 6.5$; modern window
2{,}041 events; GK: 2{,}017 mainshocks.
$N_\mathrm{series}=27$ (modern); 142 window candidates before merge; 47 merged
(all epochs).

\begin{table}[htbp]
\centering
\caption{ETAS validation (canonical --- sole full report of $p_\mathrm{ETAS}$ / $N_\mathrm{obs}$).}
\label{tab:etas_validation}
\begin{tabular}{lrrr}
\hline
Split & $N_\mathrm{obs}$ & mean & $p_\mathrm{ETAS}$ \\
\hline
In-sample (1973--2026; $n=1000$, seed~$=42$) & 27 & 27.0 & 1.0 \\
Train-calibrated hold-out (2001--2026) & 13 & 13.0 & 1.0 \\
\hline
\end{tabular}
\end{table}

\noindent Hold-out complements in-sample calibration (\S\ref{subsec:holdout-methods});
partial out-of-time check, not spatial validation.
Permutation diagnostic ($p=0.0001$)---Methods only; not a detector significance claim.

\begin{table}[htbp]
\centering
\caption{Multiplicity correction over the 142-window discovery search.}
\label{tab:bonferroni_142}
\begin{tabular}{lrr}
\hline
Correction & Threshold & Significant windows/series \\
\hline
Bonferroni & $0.05/142 \approx 3.5\times 10^{-4}$ & 0 \\
BH FDR & $q = 0.05$ & 0 \\
\hline
\end{tabular}
\end{table}

\noindent After multiplicity correction, \textbf{no claim} of significant temporal
anomalies from the window search (\texttt{results/bonferroni\_142\_windows.json}).
Post-hoc BH on 47 merged series (45/47 at $q=0.05$) is exploratory only.

\noindent\fbox{\parbox{0.97\linewidth}{\small\textbf{Multiple testing (discovery search).}
The 27 modern series derive from \textbf{142 sliding-window candidates}
($\to$ 47 merged across epochs).
\textbf{$N=27$ is NOT multiplicity-corrected} for that search.
Bonferroni $\alpha/142 \approx 3.5\times 10^{-4}$ exceeds the global permutation
$p=0.0001$ (1/10{,}001), so window-level permutation significance \textbf{does not
survive} family-wise correction.
We report \textbf{consistency with the in-sample temporal ETAS null}
(Table~\ref{tab:etas_validation}), \textbf{not} ``significant temporal clustering
after multiplicity correction.''
FDR on 47 merged series (45/47 post-hoc at $q=0.05$) is \textbf{exploratory only}---not
a primary claim (\texttt{results/fdr\_windows.json}).}}
...
\begin{tabular}{lrrrr}
\hline
Period & Events & Series & Perm.\ $p^\dagger$ & $z$-score \\
\hline
...
\noindent$^\dagger$Permutation $p$ (diagnostic only; Methods)---not primary validation.
...
not validated multi-regional triggering chains (interpretation --- \S\ref{sec:dual-etas-interpret}).
...
\noindent\textbf{Multiple-testing caveat.}
The 27 modern series derive from a search over 142 sliding-window candidates;
\textbf{FDR correction for this 142-window search was not applied} to the modern count.
A conservative Bonferroni threshold $\alpha/142 \approx 3.5\times 10^{-4}$ exceeds
the global permutation $p=0.0001$ (1/10{,}001).
Post-hoc Benjamini--Hochberg on $N=47$ merged-series $p$-values does \textbf{not}
correct the 27 modern candidates for the window search --- not a discovery claim
(\texttt{results/fdr\_windows.json}).
...
see Table~\ref{tab:etas_validation}; Figure~\ref{fig:etas_null};
interpretation --- \S\ref{sec:dual-etas-interpret}.

\subsubsection{Hold-out ETAS validation}
\label{subsubsec:etas_holdout}

Results in Table~\ref{tab:etas_validation} (train 1973--2000, hold-out 2001--2026;
\texttt{results/etas\_holdout\_validation.json}).
Does not remove in-sample coupling on the full modern window.
...
--- temporal clustering in the catalog, \textbf{not}
proof of multi-regional global chains (full $p$ --- Table~\ref{tab:epochs},
\S\ref{subsec:primary-counts}).
The \textbf{primary catalog-calibrated ETAS null} (temporal MLE): observed
candidate count is \textbf{consistent with} locally clustered synthetic
catalogs (Table~\ref{tab:etas_validation}) --- the window detector does not find
excess series beyond the calibrated null.
...
GK mainshocks (Table~\ref{tab:etas_primary}): see Table~\ref{tab:etas_validation}.
...
mechanism and Bird (2003) excluded from primary inference (no synthetic benchmark
in this work; see \S\ref{sec:limitations}).

\paragraph{WLS negative control (Supplementary S2).}
Catalog-matched WLS calibration on the same catalog illustrates
...
\textbf{temporal} clustering under the ETAS null used (Table~\ref{tab:etas_validation}):
no excess series count in detector windows relative to catalog-calibrated ETAS.
...
cluster count does not differ from the calibrated null (Table~\ref{tab:etas_validation}).
...
\paragraph{Layer 2 --- ETAS (temporal MLE).}
Fails to reject $H_0$: $N_\mathrm{series} \le$ synthetic from catalog-calibrated
ETAS --- no excess \textbf{temporal}
clustering in detector windows (Table~\ref{tab:etas_validation}).
Validity depends on temporal-only MLE (no spatial kernel).
...
Global-series hypothesis & --- & Not tested (spatial ETAS open) & Temporal-only ETAS; dispersed candidates \\
\hline
\end{tabular}
\end{table}
...
GK mainshocks --- see Table~\ref{tab:etas_validation}.
...
\textbf{In-sample ETAS:} report as consistent with the
\textbf{in-sample temporal null} (Table~\ref{tab:etas_validation}), not as proof of no anomalies.
Hold-out --- partial out-of-time check (\S\ref{subsec:holdout-methods}).
ETAS MLE on GK mainshocks: triggering model vs.\ declustered mainshocks---acknowledged mismatch.
Mean pairwise GC $> 1500$~km is weaker than an all-pairs constraint.
FDR: post-hoc BH on 47 merged series (45/47 at $q=0.05$); not applied to 142
correlated window tests (\texttt{results/fdr\_windows.json}).
Rejecting ``global series'' as a \textbf{tested} null would require spatial ETAS
with an explicit long-range kernel (Ogata 1998); we \textbf{do not} claim that
rejection here.
Spatial Ogata (1998) MLE and synthetic Bird-geometry benchmark---future work
(\texttt{docs/future\_work\_etas\_mle.md}).
...
$b=1.0$ vs $0.911$ in $\eta$ & $\eta$ upstream & Full pipeline re-run: $N_\mathrm{series}=27$ both; 8.2\% upstream label mismatch \\
...
Holding detector gates fixed, a full upstream re-run at catalog $b=0.911$
(\texttt{results/sensitivity\_b0911\_full\_pipeline.json}) leaves
$N_\mathrm{series}=27$ unchanged but shifts cluster labels for 8.2\% of events.
...
\item \textbf{$\eta$ parameters:} $b=1.0$, $d_f=1.6$ --- BP (2004) convention;
      ...
      full pipeline at $b=0.911$ $\to$ $N_\mathrm{series}=27$, Jaccard $=1.0$,
      8.2\% upstream label mismatch
      (\texttt{sensitivity\_b0911\_full\_pipeline.json}).
...
Sliding windows (1, 2, 5~yr) yield 142 cluster candidates before merge
...
\paragraph{Summary conclusions.}
We found no evidence of temporal anomalies after Bonferroni-corrected window tests;
the spatial question remains open.
Under catalog-calibrated temporal MLE, detector output is consistent with the
in-sample and hold-out ETAS null ($p_\mathrm{ETAS}=1.0$).
Permutation rejects Poisson times only---diagnostic (Methods), not global chains.
Bird and WLS excluded from primary pipeline (\texttt{paper/supplementary.md}).
...
$b$ overlap (full pipeline) & Jaccard$=1.0$; upstream 8.2\%$^{\ddagger}$ & 27 \\
...
\noindent$^{\ddagger}$\textbf{8.2\%} = 165/2017 GK mainshocks whose \texttt{identify\_clusters()}
cluster label changes when $b$ goes from 1.0 to 0.911 (full re-run GK $\to$
\texttt{identify\_clusters()} $\to$ \texttt{global\_series()};
\texttt{results/sensitivity\_b0911\_full\_pipeline.json}).

\medskip
\noindent The row \emph{b overlap (full pipeline)} reports a full pipeline re-run at
$b=1.0$ vs $b=0.911$.
\textbf{Jaccard$=1.0$} means identical \textbf{series event sets} across the 27
detections --- \texttt{global\_series()} does not use $b$ in $\eta$.
\textbf{8.2\%} upstream is the fraction of GK mainshocks whose \textbf{$\eta$-cluster
label} changes at \texttt{identify\_clusters()}; this is a different level from
downstream series membership, so equal $N_\mathrm{series}=27$ and Jaccard$=1.0$ do
\textbf{not} prove unchanged upstream structure.
...
Bird (2003) heuristic, catalog-matched WLS negative control, and pre-1900 NOAA
records are documented in \texttt{paper/supplementary.md} (\S S1--S3).
