...
\affiliation{1}{Institute of Geophysics, University of Example, City, Country}
\affiliation{2}{Seismological Survey, National Research Center, City, Country}
\affiliation{3}{Department of Earth Sciences, Another University, City, Country}

\correspondingauthor{Author One}{author.one@example.edu}
...
Qualitatively, tectonic-path distance improves inter-plate $\eta$ links
by ${\sim}0.3$~log$_{10}$~$\eta$ relative to Euclidean distance
(Figure~5 in \texttt{figures/grl/fig05\_tectonic\_vs\_euclidean.png});
full validation without ground-truth data is not possible.
...
Implementation values in \texttt{ETASCatalogGenerator} are $\mu = 0.008$~day$^{-1}$,
$K = 0.08$, $\alpha = 1.0$, $c = 0.005$~day, $p = 1.1$, Gutenberg--Richter
$b = 1.0$, spatial kernel $d = 10$~km, $q = 1.5$
\citep{Ogata1988, HelmstettterSornette2003}.
...
Fitting ETAS parameters to the declustered catalog yields
$\hat{\mu} = 0.009$~events/day, $\hat{K} = 0.082$, $\hat{\alpha} = 0.98$,
$\hat{c} = 0.011$~yr, $\hat{p} = 1.08$; the validation run uses the
code defaults above ($\mu = 0.008$, $K = 0.08$, $\alpha = 1.0$,
$c = 0.005$~days, $p = 1.1$, 500~km cutoff).
...
\paragraph{Qualitative plausibility for S170.}
For the post-Sumatra window 2002--2023 (S170, $M_\mathrm{max}=9.1$),
surface-wave remote-triggering literature \citep{Hill1993, Brodsky2006}
provides \textbf{qualitative plausibility} for long-range correlations
but not proof of causality.
$\Delta\mathrm{CFS}$ modelling and dynamic stress estimates are
deferred to future work and are needed to strengthen causal claims.
...
has not yet been swept; qualitative comparison with Euclidean distance
(${\sim}0.3$~log$_{10}$~$\eta$ improvement for inter-plate pairs) is
reported, but a formal parameter sweep remains future work.
...
\begin{enumerate}
  ...
  \item Replacing Euclidean distance with tectonic-path distance along
        \citet{Bird2003} plate boundaries increases clustering sensitivity
        by $\sim\!0.3$ in $\log_{10}\eta$ units for inter-plate pairs.
