...
27 in the modern period (1973--2026, $p = 0.0001$ (1/10{,}001 permutations)\footnote{Discrete
permutation test ($n=10{,}000$): $p = (k+1)/(n+1)$ with $k=0$ null replicates
...
links $>500$~km) on the modern window ($N_\mathrm{obs}=27$ series) uses
\textbf{catalog-calibrated} parameters from \texttt{scripts/calibrate\_etas.py}
(GK declustering + Omori MLE on 2041 modern events):
$\mu \approx 0.103$~day$^{-1}$, $K \approx 0.495$, $\alpha \approx 0.063$,
$c \approx 10^{-4}$~day, $p \approx 1.36$.
FPR~=~1000/1000; mean~=~27.0; $p_\mathrm{ETAS} = 1.0$ --- observed count
\textbf{indistinguishable} from the calibrated local-aftershock null.
Literature defaults ($\mu = 0.008$, $K = 0.08$) yield mean~$\approx 15.4$,
$p_\mathrm{ETAS} \le 0.001$ --- \textbf{comparison only}, not primary inference.
