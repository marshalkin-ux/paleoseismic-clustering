...
\noindent\textbf{WLS negative control} ($p_\mathrm{ETAS}=1.0$, mean $= N_\mathrm{obs}$):
catalog-matched calibration on 24 GK aftershocks is \textbf{invalid for inference};
\textbf{do not cite} as independent falsification (§\ref{sec:etas-calib-limits}).
...
ETAS WLS ($n=1000$, catalog) & Detector--calibration coupling & $p_\mathrm{ETAS}=1.0$: negative control only; \textbf{not} independent falsification \\
...
\item \textbf{$\eta_0$ not applied} in \texttt{global\_series()} path
      (\texttt{results/sensitivity\_b\_eta0.json}).
\item \textbf{$b=1.0$ vs catalog $b=0.911$} in $\eta$ metric: $N=27$ unchanged
      for both (\texttt{results/sensitivity\_b\_eta0.json}).
\item \textbf{ETAS WLS negative control} --- mean $= 27.0$ matches
      ...
      \textbf{Do not cite} $p_\mathrm{ETAS}=1.0$ as falsification.
