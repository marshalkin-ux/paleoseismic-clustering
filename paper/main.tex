...
literature ETAS expectation --- deviation from literature ETAS indicates clustering
this null does not fully describe; spatial analysis of ETAS residuals was
\textbf{not conducted}; the deviation is \textbf{not interpreted} as evidence for
global series; circum-Pacific concentration, \textbf{possibly} regional aftershocks,
catalog incompleteness, or detector liberalness (142 windows), but not global
chains beyond model scope
(Table~\ref{tab:etas_primary}, Table~\ref{tab:etas_decision}).
...
\begin{keypoints}
  ...
  \item \textbf{Primary ETAS null} (H\&S 2003, Table~\ref{tab:etas_primary}):
        mean $\approx 15.4$, $p_\mathrm{ETAS}\le 0.001$ --- $N_\mathrm{obs}=27$
        exceeds literature ETAS expectation; clustering not fully explained
        by model; ETAS residuals not analyzed; not evidence for global series;
        circum-Pacific, not global chains.
...
\noindent The Bird (2003) tectonic-path heuristic is a \textbf{deprecated
diagnostic only} (Appendix figures); \textbf{primary analysis uses
...
\item \textbf{ETAS local-clustering null} (primary: literature H\&S 2003,
      $\mu=0.008$, $K=0.08$; \textbf{decoupled} from detector output):
      synthetic catalogs with local triggering only ($\le 500$~km).
      $p_\mathrm{ETAS}\le 0.001$, mean $\approx 15.4$, $N_\mathrm{obs}=27$:
      observed candidate count exceeds \textbf{local aftershock-only}
      expectation---clustering beyond Poisson times requires explanation
      (aftershock structure, detector liberalness); circum-Pacific
      concentration, possibly regional aftershocks, but not global chains
      beyond model scope.
...
rejecting the permutation null does \textbf{not} contradict the primary
literature ETAS benchmark.
...
\hline
Permutation ($n=10{,}000$) & Independent event times (fixed coordinates) & Rejects temporal independence; expected for aftershock catalogs; does \textbf{not} test global series \\
ETAS literature ($n=1000$, H\&S 2003) & Local aftershock-only synthetics & $p_\mathrm{ETAS}\le 0.001$: $N_\mathrm{obs}=27 > \bar{N}\approx 15.4$; clustering beyond literature ETAS; residuals not analyzed; not evidence for global series; circum-Pacific; not global chains \\
...
\textbf{Primary inference} uses fixed literature parameters
(Helmstetter \& Sornette 2003: $\mu=0.008$, $K=0.08$, $\alpha=1.0$,
$c=0.005$~day, $p=1.1$), \textbf{decoupled} from detector output
(Table~\ref{tab:etas_primary}).
Catalog-matched calibration scripts (\texttt{scripts/calibrate\_etas.py},
\texttt{results/etas\_calibration.json}) are retained for \textbf{reproducibility
only} as a negative control (Appendix~\ref{app:wls_control}); they are
\textbf{not} publication-grade spatial Ogata (1998) MLE
(\texttt{docs/future\_work\_etas\_mle.md}).
\paragraph{Why temporal literature ETAS (H\&S 2003) as a decoupled benchmark.}
At global $M \ge 6.5$ scale, calibration is limited to \textbf{24 aftershock delays}
after GK declustering; the pipeline has \textbf{no Ogata (1998) spatial kernel} ---
only temporal branching with a 500~km cutoff.
Full spatial MLE with confidence intervals remains future work
(\texttt{docs/future\_work\_etas\_mle.md}).
Literature H\&S 2003 is a \textbf{reasoned} decoupled benchmark for
regional/global comparison, not merely a listed limitation.
...
\textbf{primary literature null} (H\&S 2003), each with
$n_\mathrm{background} \approx \mu T$ events matching the observed span,
but with no spatial coupling beyond 500~km.
On literature ETAS nulls: mean false series $\approx 15.4$ (SD~=~1.65),
max~$24$, $p_\mathrm{ETAS} \le 0.001$ --- $N_\mathrm{obs}=27$ \textbf{exceeds}
local aftershock-only expectation.
\textbf{Sensitivity (fixed $\alpha = b = 0.911$):}
\texttt{results/etas\_validation\_b0911.json} --- qualitative conclusion
unchanged ($p_\mathrm{ETAS} \le 0.001$ under literature null).
The global-series hypothesis remains \textbf{not supported}.
Table~\ref{tab:etas_primary} summarises the primary null.

\begin{table}[htbp]
\centering
\caption{Primary ETAS null model (modern window; $N_\mathrm{obs}=27$).}
\label{tab:etas_primary}
\begin{tabular}{lrrrrp{4.5cm}}
\hline
Null model & $\mu$ & $K$ & Mean & $p_\mathrm{ETAS}$ & Interpretation \\
\hline
Literature (H\&S 2003) & 0.008 & 0.08 & $\approx 15.4$ & $\le 0.001$ &
  $N_\mathrm{obs}$ exceeds literature ETAS expectation; clustering not fully
  explained by model; ETAS residuals not analyzed; not evidence for global
  series; circum-Pacific; possible regional aftershocks/incompleteness/liberalness;
  not global chains \\
\hline
\end{tabular}
\end{table}
...
not validated multi-regional triggering chains. Primary ETAS null (H\&S 2003):
$p_\mathrm{ETAS}\le 0.001$, mean $\approx 15.4$.
...
This \textbf{empirical stability} does \textbf{not} imply declustering is
philosophically immaterial: GK and ZBZ label different mainshock sets
(24 vs 1 dependent events), which could matter for cluster membership,
$\eta$ forests, or downstream cluster labelling.
...
1000 realistic ETAS catalogs under the \textbf{primary literature null}
(H\&S 2003; \citep{Ogata1988, Ogata1998, HelmstettterSornette2003}).
...
\textbf{Primary literature null} ($\mu = 0.008$, $K = 0.08$):
$\bar{N}_\mathrm{ETAS} = 15.44$ (SD~=~1.65), max~$24$,
$p_\mathrm{ETAS} \le 0.001$ --- $N_\mathrm{obs}=27$ \textbf{significantly exceeds}
local aftershock-only expectation --- deviation from ETAS indicates clustering
the model does not fully explain; circum-Pacific, possibly regional aftershocks,
not global chains beyond model scope).
\textbf{Fixed $\alpha = b = 0.911$} (base-10 branching term;
\texttt{results/etas\_calibration\_b0911.json}): qualitative conclusion
unchanged under literature null.
The detector is liberal under all parameter sets tested
...
\caption{ETAS null-model validation (primary literature null, H\&S 2003).
...
series (red line). Inset: contrast between maximum ETAS replicate (24) and
the real catalog. $p_\mathrm{ETAS} \le 0.001$; $\bar{N}_\mathrm{ETAS}\approx 15.4$.}
\label{fig:etas_null}
\end{figure}
...
($p = 0.0001$, 1/10{,}001).
The \textbf{primary literature ETAS null} (H\&S 2003) yields
$p_\mathrm{ETAS}\le 0.001$ (mean $\approx 15.4$, $N_\mathrm{obs}=27$):
observed candidate count \textbf{significantly exceeds} local aftershock-only
expectation --- deviation from literature ETAS indicates clustering this null
does not fully describe; spatial analysis of ETAS residuals was \textbf{not
conducted}; the deviation is \textbf{not interpreted} as evidence for global
series; circum-Pacific concentration, \textbf{possibly} regional aftershocks,
catalog incompleteness, or detector liberalness (142 windows), but not global
chains beyond model scope.
...
intervals; the \textbf{primary hypothesis-test null} uses fixed literature
parameters (Table~\ref{tab:etas_primary}): mean $\approx 15.4$,
$p_\mathrm{ETAS}\le 0.001$ --- $N_\mathrm{obs}=27$ \textbf{exceeds} local
aftershock-only expectation; deviation from ETAS indicates clustering the
model does not fully explain---circum-Pacific, possibly regional aftershocks,
but not global chains beyond model scope (Table~\ref{tab:etas_decision}).
Catalog-matched calibration details are a reproducibility negative control
only (Appendix~\ref{app:wls_control}).
...
the global multi-regional series hypothesis is \textbf{not confirmed}
(literature ETAS: $p_\mathrm{ETAS}\le 0.001$ indicates clustering the model
does not fully explain; circum-Pacific, possibly regional aftershocks, not
global chains beyond model scope).
...
the rejection is \textbf{statistically real}: $N_\mathrm{obs}=27$ significantly
exceeds local aftershock-only ETAS expectation under H\&S 2003 parameters.
Deviation from literature ETAS indicates clustering this null does not fully
explain; spatial analysis of ETAS residuals was \textbf{not conducted}; the
deviation is \textbf{not interpreted} as evidence for global series; spatial
analysis of candidates shows concentration within the circum-Pacific belt,
\textbf{possibly} reflecting regional aftershock sequences, catalog
incompleteness, or detector liberalness (142 windows), but not global chains
beyond model scope. Quantitative excess also reflects detector rules (mean GC $>1500$~km,
$N\ge 4$) and \textbf{liberal exploratory search} (142 sliding windows before
merge).
...
\begin{table}[htbp]
\centering
\caption{Decision tree: primary ETAS null interpretation (modern window).}
\label{tab:etas_decision}
\small
\begin{tabular}{p{3.2cm}p{2.2cm}p{2.5cm}p{5.8cm}}
\hline
...
Literature H\&S 2003 (primary) & $\le 0.001$ & Exceeds (mean $\approx 15.4$) &
  Clustering not explained by ETAS; residuals not analyzed; not evidence for
  global series; circum-Pacific; possible regional aftershocks/incompleteness/liberalness; not global chains \\
...
$N_\mathrm{obs}=27$ significantly exceeds local aftershock-only expectation.
...
\hline
$\eta_0$ unverified at global scale & GK declustering & $N=27$ unchanged (\texttt{global\_series} does not use $\eta_0$) \\
...
Literature $p\le 0.001$ & ETAS test & Clustering not explained by ETAS; circum-Pacific; not global chains \\
\hline
\end{tabular}
\end{table}
...
Without spatial Ogata (1998) MLE, the ETAS null relies on literature H\&S 2003
parameters only; catalog-matched fits in Appendix~\ref{app:wls_control} are
reproducibility controls, not publication-grade inference.
The 142 sliding-window candidates merged to 47 series are the dominant source
of detector liberalness; literature $p_\mathrm{ETAS}\le 0.001$ indicates a
clustering the ETAS model does not fully explain (circum-Pacific, possibly
regional aftershocks) but does \textbf{not} overturn the null on the
global-series hypothesis (no mechanism, GC-only gates).
...
\item \textbf{$\eta_0$ scope:} affects GK/ZBZ \texttt{identify\_clusters()},
      ...
      (\texttt{pipeline\_v2}).
\item \textbf{ETAS null:} literature H\&S 2003 only in main inference; spatial
      Ogata MLE --- future work (\texttt{docs/future\_work\_etas\_mle.md});
      catalog calibration --- Appendix~\ref{app:wls_control} (reproducibility).
...
Under the \textbf{primary literature ETAS null} (H\&S 2003), $N_\mathrm{obs}=27$
exceeds mean $\approx 15.4$ ($p_\mathrm{ETAS}\le 0.001$) --- clustering the
ETAS model does not fully explain; circum-Pacific, possibly regional aftershocks,
not global chains beyond model scope.
...
\begin{enumerate}
  ...
  \item \textbf{Primary ETAS null} (literature H\&S 2003): mean $\approx 15.4$,
        ...
        aftershock expectation; clustering not fully explained by ETAS;
        circum-Pacific, not global chains (Table~\ref{tab:etas_decision}).
  \item 47 detector candidates; global multi-regional series hypothesis
        \textbf{not confirmed} (liberal search space; no physical mechanism;
        tectonic heuristic removed from primary pipeline; great-circle only);
        $\Delta$CFS/dynamic stress --- future work only.
\end{enumerate}
