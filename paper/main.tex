...
$p_\mathrm{ETAS}=1.0$ --- $N_\mathrm{obs}$ is consistent with the \textbf{in-sample temporal null}.
Under the temporal ETAS model used, we find no anomalous temporal clustering beyond
the in-sample calibrated null; the spatial component was not modeled, so questions of
...
\textbf{Scope.} We extend prior global rate tests with a complementary
...
\texttt{results/etas\_holdout\_validation.json}) is reported in
Section~\ref{subsubsec:etas_holdout}.
...
\subsubsection{Hold-out ETAS validation (train 1973--2000, validate 2001--2026)}
\label{subsubsec:etas_holdout}

To complement the \textbf{in-sample} primary test, we fit temporal MLE on GK
mainshocks in 1973--2000 only and validate $N_\mathrm{series}$ on the hold-out
catalog 2001--2026 with train-calibrated parameters
(\texttt{scripts/calibrate\_etas\_holdout.py},
\texttt{results/etas\_holdout\_validation.json}; $n=1000$ synthetics,
\texttt{max\_total\_events}=5000).
Hold-out: $N_\mathrm{obs}=13$, synthetic mean $= 13.0$, $p=1.0$ --- also
\textbf{consistent with the train-calibrated temporal null} on the shorter
hold-out span (25~yr).
This does not remove the in-sample coupling of calibration and detector on the
full modern window; it provides a partial out-of-time check.
...
\textbf{In-sample ETAS:} $p_\mathrm{ETAS}=1.0$ on 1973--2026 uses calibration and
the same detector on the same window---report as consistent with the
\textbf{in-sample temporal null}, not as proof of no anomalies.
Hold-out train/test (1973--2000 / 2001--2026): $N_\mathrm{obs}=13$, mean $=13.0$,
$p=1.0$ (\texttt{results/etas\_holdout\_validation.json})---partial out-of-time check.
