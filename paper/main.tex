...
\title{Global Seismic Series in Paleoseismic and Historical Catalogs:
       A Clustering Analysis Using the Baiesi–Paczuski Metric}
...
%% =====================================================================
\begin{abstract}
We present a systematic analysis of global seismic series spanning
from 2150~BCE to 2026~CE, combining the NOAA~NGDC historical catalog,
the USGS~ComCat instrumental catalog, and the ISC~Bulletin into a unified
catalog of \textbf{4{,}267 unique $M \ge 6.5$ events} (from 4{,}418 saved
CSV rows; ${\sim}151$ NOAA records with $M < 6.5$ from an $M \ge 6.0$ fetch
are \textbf{retained in CSV for provenance but excluded from all clustering
analyses}; 2{,}041 modern, 2{,}179 early instrumental, 47 pre-1900).
Using the nearest-neighbor metric of \citet{Baiesi2004}, extended with
a tectonic-path distance derived from the \citet{Bird2003} plate-boundary
graph, we identify 47 global seismic series in three temporal windows:
27 in the modern period (1973--2026, $p < 0.0001$, $z = -6.17$),
15 in the early instrumental period (1900--1972, $p = 0.007$, $z = -2.43$),
and 5 historical episodes (pre-1900).
Significance is assessed by a three-level statistical framework:
(i)~a permutation test ($n = 10{,}000$ surrogates) against a
spatially inhomogeneous Poisson null model;
(ii)~an Epidemic-Type Aftershock Sequence (ETAS) validation using
100 synthetic catalogs calibrated to the observed $b$-value and
magnitude distribution, providing a null model that explicitly
reproduces local aftershock clustering without long-range coupling;
and (iii)~the Benjamini--Hochberg false discovery rate (FDR) correction
\citep{BenjaminiHochberg1995} at $q = 0.05$ applied to all candidate
series $p$-values.
ETAS null-model validation (100 synthetic catalogs without long-range
links $>500$~km) yields $p_\mathrm{ETAS} < 10^{-4}$ (FPR~=~0), confirming
that observed series cannot be attributed to local aftershock clustering.
After Benjamini--Hochberg FDR correction ($q = 0.05$), 45 of 47 series
remain significant ($q$-values spanning $[9.7 \times 10^{-5},\, 0.047]$).
Background seismicity is isolated using Gardner--Knopoff declustering
as the \textbf{primary} pre-processing step (\texttt{pipeline\_v2.py});
Zaliapin--Ben-Zion declustering is a \textbf{sensitivity check only},
not a sequential filter.
The largest series by event count is 1905--1910 (193 events, 43 regions,
$M_\mathrm{max}=8.8$); the most spatially extensive is S170 (46 events,
12 Flinn--Engdahl regions, 2002--2023, $M_\mathrm{max}=9.1$),
which includes the 2004 Indian~Ocean earthquake.
Series detection is a \textbf{statistical anomaly} ($\eta$ links,
$p$-values); causal mechanism is not established.
These results have implications for probabilistic seismic hazard assessment
and for models of stress interaction at tectonic plate boundaries.
\end{abstract}

\begin{keypoints}
  \item A unified catalog of \textbf{4{,}267 unique $M \ge 6.5$ events}
        (4{,}418 CSV rows; ${\sim}151$ sub-threshold rows excluded from analysis)
        yields 47 global seismic series validated by a three-level statistical framework:
        permutation test, ETAS-null validation, and Benjamini--Hochberg FDR correction.
  \item Modern period (1973--2026): $p < 0.0001$ ($z = -6.17$); early instrumental
        (1900--1972): $p = 0.007$ ($z = -2.43$); both survive FDR correction at $q = 0.05$
        and are incompatible with a Poissonian or ETAS null model without long-range coupling.
  \item The largest series (1905--1910: 193 events, 43 regions, $M_\mathrm{max}=8.8$) and
        most spatially extensive (S170: 12 regions, 18{,}100~km, $M_\mathrm{max}=9.1$,
        2002--2023) indicate distinct clustering regimes driven by mechanisms beyond local
...
spanning from 2150~BCE to 2024~CE, with events $M \ge 6.5$,
and search for \emph{global} series---episodes in which $\ge 4$
events occur in $\ge 3$ distinct Flinn--Engdahl regions within a
1-year window.
...
events from approximately 2150~BCE, based on documentary sources
...
4{,}267 unique events remain from 4{,}418 saved CSV rows spanning 2150~BCE to
2026~CE: 2{,}041 modern (1973--2026), 2{,}179 early instrumental
...
of the parent event.
Note: $b = 1.0$ follows the original \citet{Baiesi2004} formulation for
cross-study $\eta$ comparability; the catalog's empirical $b$-value is
$0.911 \pm 0.018$ (used in the Monte~Carlo null and completeness analysis,
not in the $\eta$ formula).
...
$\to$ series criteria ($N \ge 4$, $\ge 3$ Flinn--Engdahl regions)
...
events with $M \ge 6.5$ occurring within a 1-year temporal
window and spanning $\ge 3$ distinct Flinn--Engdahl regions.
The window is advanced in steps of one year, and overlapping groups
are merged.
...
Implementation values in \texttt{ETASCatalogGenerator} are \textbf{default
global parameters} \citep{Ogata1988, HelmstettterSornette2003}:
$\mu = 0.008$~day$^{-1}$, $K = 0.08$, $\alpha = 1.0$, $c = 0.005$~day,
$p = 1.1$, Gutenberg--Richter $b = 1.0$---\textbf{not} re-calibrated on the
2041-event modern catalog (\texttt{results/etas\_calibration\_note.md}).
Optional homogeneous Poisson MLE on catalog times yields
$\mu \approx 0.105$~events/day vs the default 0.008; full spatial ETAS MLE
and multi-seed robustness are future work.
...
We generate 100 synthetic ETAS catalogs (seed~=~42), each containing the
same number of events as the observed catalog and matching its
magnitude--frequency distribution, but with no spatial coupling beyond
500~km.
The global-series algorithm ($N \ge 4$,
$\ge 3$ Flinn--Engdahl regions, 2-yr window) is applied to each
synthetic catalog, yielding a null distribution for the number of
detected series.
The false-positive rate (FPR) is the fraction of catalogs with
$\ge 1$ spurious global series: mean false series
$= 0.00 \pm 0.00$, FPR~=~0/100, $n_\mathrm{obs} = 47$.
\textbf{Limitation:} FPR~=~0/100 at fixed seed~=~42 is a discrete outcome;
multi-seed robustness is recommended as future work; rejecting the ETAS
null does not prove a physical triggering mechanism.
The ETAS $p$-value is defined as $p_\mathrm{ETAS} = P(N^\mathrm{ETAS}_\mathrm{series}
\ge N^\mathrm{obs}_\mathrm{series}) = 0.0000$ (empirical resolution
$1/101$).
If $p_\mathrm{ETAS} < 0.05$, the observed series count cannot be
attributed to local aftershock structure alone.
...
The full historical analysis covers 4{,}267 events ($M \ge 6.5$, BCE--2026,
4{,}418 CSV records) and identifies 47 global seismic series in three
temporal windows (Table~\ref{tab:epochs}).
...
\hline
Series & $N$ & Regions & $M_\mathrm{max}$ & Period & Note \\
\hline
1905--1910 & 193 & 43 & 8.8 & 1905--1910 & Largest ever \\
1960--1965 & 166 & 35 & 9.2 & 1960--1965 & Chile 1960 ($M~9.2$) \\
2011--2013 &  87 & 32 & 8.6 & 2011--2013 & Tohoku 2011 \\
S170       &  46 & 12 & 9.1 & 2002--2023 & Sumatra 2004 ($M~9.1$) \\
856--887 CE &  6 &  4 & 8.6 & 856--887   & Medieval cluster \\
\hline
\end{tabular}
\end{table}
...
$z = -6.17$, $p < 0.0001$ for the modern period.
...
Validation uses the literature defaults above ($\mu = 0.008$, $K = 0.08$,
$\alpha = 1.0$, $c = 0.005$~days, $p = 1.1$, 500~km cutoff), not a
catalog-specific fit.
The mean number of global series detected in 100 ETAS synthetic catalogs
is $\bar{N}_\mathrm{ETAS} = 0.00$ (SD~=~0.00); the false-positive rate
(FPR, fraction of catalogs with $\ge 1$ global series) is 0/100.
The ETAS $p$-value is $p_\mathrm{ETAS} = 0.0000$ (empirical resolution
$1/101$---a discrete test that rejects the local-only null but does not
identify the physical triggering mechanism).  Because none of the 100 ETAS catalogs produced a global series,
the observed 47 candidates cannot be attributed to local aftershock dynamics
alone and represent a statistically significant departure from the standard
ETAS null model without long-range coupling (Figure~\ref{fig:etas_null}).
...
series detected in each of 100 synthetic ETAS catalogs (blue; all zero).
The observed catalog yields 47 candidate series (red annotation).
Inset: contrast between maximum ETAS replicate and the real catalog.
$p_\mathrm{ETAS} = 0.0000$; FPR~=~0/100.}
\label{fig:etas_null}
\end{figure}
...
The observed value lies $6.17\sigma$ below the null mean ($p < 0.0001$).}
...
series ($\geq 3$ Flinn--Engdahl regions).  Marker size is proportional
to magnitude.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{../figures/grl/fig06_cfs_s170.png}
\caption{Post-Sumatra 2004 static $\Delta\mathrm{CFS}$ for S170 receivers
(Japan and Aleutians, 2004--2016; Okada 1985, $\mu=0.4$).}
\label{fig:cfs_s170}
\end{figure}
...
\paragraph{Statistical anomaly (established).}
...
null model.
The ETAS validation provides an independent and more stringent test:
because $p_\mathrm{ETAS} = 0.0000$ with zero false positives in 100
replicates (seed~=~42), the observed multi-regional series cannot be
explained by standard aftershock branching alone within 500~km.
This is a conclusion about \textbf{$\eta$ links and $p$-values}, not
causality.

\paragraph{Physical mechanism (not established).}
The $\eta$ metric is correlative; hypocentral depth, focal mechanisms,
and $\Delta\mathrm{CFS}$ are not incorporated.
\textbf{Series detection does not imply direct triggering;} co-occurrence
may reflect shared tectonic loading, mantle coupling, or catalog
incompleteness artifacts.
Either a genuine long-range coupling mechanism operates at global
scales (e.g., viscoelastic relaxation or dynamic stress transfer), or
current ETAS formulations underestimate large-scale temporal coherence
--- a question that $\Delta\mathrm{CFS}$ modelling can address
(Section~\ref{subsec:perspectives}).
Three physical mechanisms \emph{may} account for inter-event
coupling over the tectonic distances recovered here, but none is
established by the present statistical analysis alone.
\paragraph{Early instrumental period (1900--1972).}
Fifteen series in this window reach statistical significance
($p = 0.007$, $z = -2.43$), but most pre-1960 events have
quality-score~$< 0.7$, requiring cautious interpretation of individual
episodes (including 1905--1910).

\paragraph{Static $\Delta\mathrm{CFS}$ for S170 (Sumatra 2004).}
We computed static Coulomb stress change (Okada, 1985; $\mu = 0.4$) for
\textbf{79 receivers} (2004--2016) in Japan and the Aleutians
(\texttt{scripts/compute\_cfs\_s170.py},
\texttt{results/cfs\_s170\_analysis.json},
Figure~\ref{fig:cfs_s170}).
Source: 26~Dec~2004 $M~9.1$ at $3.3^\circ\mathrm{N}$, $95.8^\circ\mathrm{E}$
(GCMT: strike $320^\circ$, dip $10^\circ$, rake $110^\circ$).
Japan ($n=63$): mean $\Delta\mathrm{CFS} = +0.008$~kPa, median $+0.007$~kPa,
range 0.003--0.023~kPa, 100\% promoting on a regional template;
Aleutians ($n=16$): mean $+0.000065$~kPa (two orders of magnitude smaller).
These values are \textbf{$\ll 0.1$~bar} ($\sim$10~kPa), the typical static
triggering threshold \citep{Stein1999}; static Coulomb transfer alone is
\textbf{insufficient}---dynamic triggering and/or viscoelastic coupling
(§\ref{subsec:hypotheses}) remain necessary hypotheses.
...
\textbf{Critical limitation:} ETAS validation reports FPR~=~0/100 using
100 synthetic catalogs at \textbf{fixed seed~=~42} only (discrete
resolution 1/101). Multi-seed robustness (recommended: $\ge 1000$
catalogs across multiple seeds) has not been run; see
\texttt{scripts/run\_etas\_multiseed.py}. Rejecting the ETAS null does
not prove a physical triggering mechanism.
...
Formal ETAS validation has been completed ($p_\mathrm{ETAS} = 0.0000$;
Section~\ref{subsubsec:etas_results}); future work should extend this
...
to 1{,}821 events and yields 18 modern series ($p < 0.0001$), confirming
...
\begin{enumerate}
  \item Full historical analysis (4{,}267 events $M \ge 6.5$; 4{,}418 CSV
        records; BCE--2026) identifies
        47 global seismic series in three temporal windows: 27 in the modern
        period (1973--2026), 15 in the early instrumental period (1900--1972),
        and 5 historical episodes (pre-1900).
  \item The Monte~Carlo permutation test ($n_\mathrm{sim} = 10\,000$) yields
        $p < 0.0001$ ($z = -6.17$) for the modern period and $p = 0.007$
        ($z = -2.43$) for the early instrumental period, both incompatible
        with a Poissonian null hypothesis.
  \item ETAS validation (100 synthetic catalogs without long-range coupling)
        yields $p_\mathrm{ETAS} = 0.0000$ (FPR~=~0); together with
        Benjamini--Hochberg FDR correction ($q = 0.05$, 45/47 series
        significant), this confirms that detected series are not artefacts
        of randomness or local aftershock structure.
  \item Tectonic-path distance: 2.0\% of 4987 audited pairs use Dijkstra,
        98.0\% the 1.5$\times$ GC fallback (500 events sampled;
        \texttt{scripts/analyze\_tectonic\_fallback.py}); metric adds value
        mainly for boundary-proximal pairs; median $\Delta\log_{10}\eta = +0.28$.
  \item Static $\Delta\mathrm{CFS}$ after Sumatra 2004 (S170): Japan
        $+0.008$~kPa ($n=63$), Aleutians $+0.000065$~kPa ($n=16$)---promoting
        but $\ll 0.1$~bar; dynamic/viscoelastic mechanisms needed.
  \item The largest series by event count is 1905--1910 (193 events,
        43 regions, $M_\mathrm{max}=8.8$); the most spatially extensive
        modern chain is S170 (46 events, 12 regions, 2002--2023,
        $M_\mathrm{max}=9.1$), encompassing the 2004 Indian Ocean
        earthquake and a decade-long activation of the Circum-Pacific belt.
  \item Given current evidence, two interpretive paths remain open
        (statistics vs.\ mechanism):
        \textbf{(a)}~if $\eta$ links reflect genuine spatiotemporal structure,
        hazard models may need long-range ETAS kernels (without claiming
        direct triggering);
        \textbf{(b)}~if episodes prove artifactual, the FDR~+~ETAS framework
        remains a reproducible null-testing pipeline.
        \textbf{Series detection does not imply direct triggering;}
        co-occurrence may reflect shared tectonic loading or mantle coupling.
\end{enumerate}
