...
\title{Global Seismic Series: Spatiotemporal Clustering in M$\ge$6.5 Catalogs (1973--2026 CE)}
...
%% =====================================================================
\begin{abstract}
We present a systematic analysis of global seismic series in a four-millennium
\textbf{4{,}267 unique $M \ge 6.5$ event} catalog (4{,}418 CSV rows; ${\sim}151$
sub-threshold NOAA rows excluded from clustering). \textbf{Significance claims
focus on the modern instrumental window (1973--2026, 2{,}041 events) and early
instrumental period (1900--1972, 2{,}179 events);} 47 pre-1900 records are
retained for context but not used for epoch-level significance.
...
27 in the modern period (1973--2026, $p \le 0.0001$\footnote{Discrete
permutation test ($n=10{,}000$): minimum achievable $p = 1/(n+1) = 0.0001$.},
$z = -6.17$),
...
1000 synthetic catalogs calibrated to the observed $b$-value and
...
\citep{BenjaminiHochberg1995} at $q = 0.05$ applied to $N = 47$ merged
series $p$-values (142 window candidates before merge).
ETAS null-model validation (1000 synthetic catalogs without long-range
links $>500$~km) on the modern window ($N_\mathrm{obs}=27$ series):
FPR~=~1000/1000 catalogs with $\ge 1$ spurious series (mean
$15.6 \pm 1.7$); $p_\mathrm{ETAS} = P(N_\mathrm{ETAS} \ge 27) = 0/1000$
($\le 0.001$), confirming
...
\begin{keypoints}
  ...
  \item Modern period (1973--2026): $p \le 0.0001$ ($z = -6.17$); early instrumental
...
We generate 1000 synthetic ETAS catalogs (seed~=~42), each containing the
...
$\ge 1$ spurious global series: FPR~=~1000/1000, mean false series
$= 15.58 \pm 1.67$, $n_\mathrm{obs} = 27$ (modern window).
\textbf{Note:} previously reported FPR~=~0/100 arose from an API mismatch
(\texttt{min\_regions}); corrected here. The detector is liberal on ETAS
nulls, but $N_\mathrm{obs}=27$ exceeds the ETAS null maximum ($\approx 19$).
The ETAS $p$-value is $p_\mathrm{ETAS} = P(N^\mathrm{ETAS}_\mathrm{series}
\ge N^\mathrm{obs}_\mathrm{series}) = 0/1000$ (report $\le 0.001$).
...
$z = -6.17$, $p \le 0.0001$ for the modern period.
...
(FPR, fraction of catalogs with $\ge 1$ global series) is 1000/1000;
mean spurious series $15.6 \pm 1.7$ per catalog.
...
series detected in each of 1000 synthetic ETAS catalogs (blue; all zero).
...
$p_\mathrm{ETAS} \le 0.001$; FPR~=~1000/1000; $N_\mathrm{obs}=27$.}
\label{fig:etas_null}
\end{figure}
...
because $p_\mathrm{ETAS} \le 0.001$ with zero false positives in 1000
...
The $\eta$ metric is correlative; hypocentral depth and focal mechanisms
are not incorporated in the primary analysis.
Preliminary Coulomb/dynamic stress tests for S170 did not reach triggering
thresholds (see repository supplement).
...
\textbf{ETAS note:} ETAS validation (1000 catalogs) reports FPR~=~1000/1000
for $\ge 1$ spurious series (mean $15.6$); $N_\mathrm{obs}=27$ exceeds ETAS
null maximum ($\approx 19$). Previously reported FPR~=~0/100 was an API bug.
Rejecting the ETAS null does not prove a physical triggering mechanism.
...
\begin{enumerate}
  ...
  \item The Monte~Carlo permutation test ($n_\mathrm{sim} = 10\,000$) yields
        $p \le 0.0001$ ($z = -6.17$) for the modern period and $p = 0.007$
        ($z = -2.43$) for the early instrumental period, both incompatible
        with a Poissonian null hypothesis.
  \item ETAS validation (1000 synthetic catalogs without long-range coupling)
        yields $p_\mathrm{ETAS} \le 0.001$ (FPR~=~1000/1000; $N_\mathrm{obs}=27$); together with
        Benjamini--Hochberg FDR correction ($q = 0.05$, $N=47$, 45/47 series
  ...
  \item Tectonic-path distance: 2.0\% of 4987 audited pairs use Dijkstra,
        ...
        \texttt{scripts/analyze\_tectonic\_fallback.py}); median $\Delta\log_{10}\eta = +0.28$.
