...
thresholds or ETAS calibration (24 GK aftershocks used in WLS).
...
\noindent Catalog-matched WLS calibration ($p_\mathrm{ETAS}=1.0$) is retained
as a \textbf{negative control only} (Appendix~\ref{app:wls_control}); \textbf{not}
used for inference.
...
\textbf{WLS is never used for inference} (negative control only).
Temporal MLE: \texttt{results/etas\_mle\_calibration.json} ($\mu \approx 0.097$,
$K \approx 10^{-4}$ at lower bound, $\alpha \approx 0.254$, $c = 0.001$~day,
$p \approx 1.91$; bootstrap 95\% CIs --- $K$, $\alpha$, $c$ pinned at bounds,
weak intervals). Spatial Ogata (1998) MLE --- \texttt{docs/future\_work\_etas\_mle.md}.
...
Catalog-matched WLS parameters ($\mu \approx 0.103$, $K \approx 0.495$) are
documented in \S\ref{subsec:etas-calibration} and Appendix~\ref{app:wls_control}
as a \textbf{negative control only} --- \textbf{not} used for inference.
...
Table~\ref{tab:etas_primary} summarises the primary null; detector liberalness
under catalog-matched calibration is in Appendix~\ref{app:wls_control}.
...
decoupled from detector). Catalog-matched WLS ($p_\mathrm{ETAS}=1.0$) is
\textbf{not} used for inference (Appendix~\ref{app:wls_control}).
...
$\eta$ forests, or ETAS WLS on 24 GK aftershock delays.
...
Catalog-matched WLS validation (mean $= 27.0$, $p_\mathrm{ETAS}=1.0$) ---
Appendix~\ref{app:wls_control} only.
...
(Table~\ref{tab:etas_primary}): mean $\approx 15.4$, $p_\mathrm{ETAS}\le 0.001$ ---
...
Catalog-matched WLS ($p_\mathrm{ETAS}=1.0$) is documented only as a negative
control (Appendix~\ref{app:wls_control}).
...
Catalog-matched WLS ($p_\mathrm{ETAS}=1.0$) --- Appendix~\ref{app:wls_control} only.
...
Catalog-matched validation ($p_\mathrm{ETAS}=1.0$, mean $= 27.0$) is retained
as a negative control only (Appendix~\ref{app:wls_control}).
...
\item \textbf{WLS ETAS calibration failure:} $K\approx 0.495$ on 24 aftershocks
      with $c$ at numerical floor ($10^{-4}$~day); \textbf{invalid for inference};
      temporal MLE attempt: \texttt{results/etas\_mle\_calibration.json}
      (see \texttt{docs/future\_work\_etas\_mle.md} for spatial Ogata MLE).
\item \textbf{ETAS WLS negative control} --- documented in
      Appendix~\ref{app:wls_control} only; \textbf{not} used for inference.
...
Catalog-matched WLS (mean $= 27.0$, $p_\mathrm{ETAS}=1.0$) --- Appendix~\ref{app:wls_control}.
...
\textbf{Do not cite} $p_\mathrm{ETAS}=1.0$ alone as falsification of global
structure.
