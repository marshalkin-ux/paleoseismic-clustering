...
27 modern) --- algorithmic outputs, not validated physical global series.
\textbf{Primary result --- negative (null) outcome:} the global multi-regional
series hypothesis is \textbf{not confirmed}; the detector is \textbf{liberal}.
\textbf{Primary ETAS null} --- catalog-calibrated temporal Ogata (1988) MLE
on 2017 GK mainshocks (1973--2026, $M\ge 6.5$; Table~\ref{tab:etas_primary}):
$\mu=0.097$, $K=10^{-4}$, $\alpha=0.25$, $c=0.001$~day, $p=1.91$;
mean $= 27.0$, $p_\mathrm{ETAS}=1.0$ ($N_\mathrm{obs}=27$): observed candidate
count is \textbf{consistent with} catalog-calibrated local ETAS --- does
\textbf{not} support excess global series beyond the null.
...
Poisson event times} for aftershock catalogs \citep{Ogata1988}; tests temporal
clustering only; \textbf{not} teleseismic/multi-regional chains.
Literature H\&S 2003 plug-in values (mean $\approx 15.4$, $p_\mathrm{ETAS}\le 0.001$)
were \textbf{invalid as primary null} and are retained for comparison only
...
\begin{keypoints}
  \item 47 detector candidates (27 modern); global multi-regional series hypothesis
        \textbf{not confirmed} (liberal search space; no physical mechanism).
  \item \textbf{Primary ETAS null} (temporal MLE, GK mainshocks, Table~\ref{tab:etas_primary}):
        mean $= 27.0$, $p_\mathrm{ETAS}=1.0$ --- $N_\mathrm{obs}=27$ consistent
        with catalog-calibrated ETAS; does not support excess global series.
...
(c) Global-series hypothesis & No teleseismic multi-regional chains beyond local clustering & Such chains exist & \textbf{Not confirmed} \\
(d) Literature H\&S (comparison) & Plug-in regional parameters & --- & mean $\approx 15.4$, $p_\mathrm{ETAS}\le 0.001$; \textbf{invalid primary null} \\
(e) WLS negative control (App.~\ref{app:wls_control}) & Detector--calibration coupling artifact & --- & $p_\mathrm{ETAS}=1.0$, mean $=27=N_\mathrm{obs}$; Appendix B only \\
\hline
\end{tabular}
\end{table}
...
against the global-series hypothesis.
...
\item \textbf{Global multi-regional series hypothesis} (specific alternative:
      mean GC $> 1500$~km, $N\ge 4$): \textbf{not confirmed}---no physical
      mechanism, tectonic metric failed as primary gate, detector liberal
      (§\ref{sec:detector-liberal}).
\end{enumerate}
...
sub-hypotheses and do \textbf{not} contradict the negative global-series conclusion.
...
ETAS literature ($n=1000$, H\&S 2003) & Plug-in regional parameters & $p_\mathrm{ETAS}\le 0.001$, $\bar{N}\approx 15.4$; \textbf{invalid primary null}; comparison only \\
...
Earlier drafts incorrectly used plug-in literature H\&S 2003 values
($\mu=0.008$, $K=0.08$) as the primary null; those values are retained for
\textbf{comparison only} (\texttt{results/etas\_validation\_literature.json}).
...
the calibrated null.
Literature H\&S 2003 (comparison): $\bar{N} \approx 15.4$,
$p_\mathrm{ETAS}\le 0.001$ --- \textbf{invalid as primary null} for this catalog.
The global-series hypothesis remains \textbf{not supported}.
...
Literature (H\&S 2003) & 0.008 & 0.08 & $\approx 15.4$ & $\le 0.001$ &
  Invalid primary null; comparison only \\
...
Literature H\&S comparison: $\bar{N} \approx 15.4$, $p_\mathrm{ETAS}\le 0.001$
...
Literature H\&S comparison ($\bar{N}\approx 15.4$) shown in text only.}
\label{fig:etas_null}
\end{figure}
...
Literature H\&S plug-in values ($\bar{N}\approx 15.4$, $p_\mathrm{ETAS}\le 0.001$)
were \textbf{invalid as primary null} and are comparison only.
...
This \textbf{supports} the negative conclusion on global series but WLS is
...
the global multi-regional series hypothesis is \textbf{not confirmed}
(temporal MLE: $p_\mathrm{ETAS}=1.0$; detector consistent with calibrated null).
...
\textbf{Primary conclusion:} the global multi-regional series hypothesis is
\textbf{not confirmed} (no mechanism, tectonic gate failed, liberal detector).
...
ETAS. $N_\mathrm{obs}=27 = \bar{N}=27.0$ --- detector output is consistent with
the calibrated null; does not support excess global series. Validity depends on
temporal-only MLE (no spatial kernel).

\paragraph{Layer 3 --- Global-series hypothesis.}
\textbf{Not confirmed}: spatial/geophysical gates fail; circum-Pacific candidates
are not validated teleseismic chains.

\paragraph{Literature comparison (invalid primary).}
H\&S 2003 plug-in values yield $\bar{N}\approx 15.4$, $p_\mathrm{ETAS}\le 0.001$ ---
\textbf{wrong null for this catalog}; retained for comparison only.
...
Global-series hypothesis & --- & Not confirmed & Primary scientific claim (negative) \\
Literature H\&S & $\le 0.001$ & Exceeds ($\bar{N}\approx 15.4$) & Invalid primary null; comparison only \\
\hline
\end{tabular}
\end{table}
...
Calibrated $p_\mathrm{ETAS}=1.0$ & ETAS test & Consistent with null; supports negative global-series conclusion \\
\hline
\end{tabular}
\end{table}
...
\textbf{not} apply $\eta_0$ filtering; the main negative conclusion is therefore
...
mainshocks; literature H\&S values are comparison only.
...
detector output matches the null and does \textbf{not} overturn the negative
global-series conclusion (no mechanism, GC-only gates).
...
\item \textbf{ETAS null:} temporal MLE on GK mainshocks (primary);
      literature H\&S and WLS --- comparison/Appendix~\ref{app:wls_control};
...
matches mean $= 27.0$ ($p_\mathrm{ETAS}=1.0$) --- detector output is
consistent with catalog-calibrated local ETAS; the global-series hypothesis
remains \textbf{not confirmed}.
...
\begin{enumerate}
  \item \textbf{Global multi-regional series hypothesis not confirmed} --- no
        physical mechanism, tectonic gate failed, liberal detector (142 windows).
  ...
  \item \textbf{Primary ETAS null} (temporal MLE, GK mainshocks): mean $= 27.0$,
        $p_\mathrm{ETAS}=1.0$ --- $N_\mathrm{obs}=27$ consistent with calibrated
        ETAS; does not support excess global series (Table~\ref{tab:etas_decision}).
...
hypothesis testing.
\textbf{Do not cite} $p_\mathrm{ETAS}=1.0$ in the abstract, conclusions, or
hero metrics.
