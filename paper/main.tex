...
{\large Temporal ETAS null and hold-out validation}}
...
The spatial component was not modeled (\S\ref{sec:limitations}).
...
\textbf{Objective.} Bound what reproducible tests can establish about
multi-regional global series in the $M \ge 6.5$ catalog (primary window
1973--2026).
...
\textbf{not tested}.
Bird (2003) tectonic distance was explored early and \textbf{excluded from
primary analysis} (Supplementary S1).
...
Under catalog-calibrated temporal MLE, no anomalous temporal clustering beyond
the null; spatial linkage was not tested.
Permutation rejects Poisson times only --- not global chains (see
\S\ref{sec:results}).
GK/ZBZ/none invariant at fixed gates reflects detector dominance
(\texttt{results/sensitivity\_declustering.json}).
