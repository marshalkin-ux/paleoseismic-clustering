...
$15.4 \pm 1.7$; max~$24$); $p_\mathrm{ETAS} = P(N_\mathrm{ETAS} \ge 27) = 0/1000$
($\le 0.001$), confirming
...
The mean number of global series detected in 1000 ETAS synthetic catalogs
is $\bar{N}_\mathrm{ETAS} = 15.44$ (SD~=~1.65); the false-positive rate
(FPR, fraction of catalogs with $\ge 1$ global series) is 1000/1000;
maximum spurious count per catalog is 24.
The ETAS $p$-value is $p_\mathrm{ETAS} = P(N_\mathrm{ETAS} \ge 27) = 0/1000$
(report $\le 0.001$).
Previously reported FPR~=~0/100 arose from an API bug (invalid
\texttt{min\_regions} argument caused TypeError and returned zero).
The detector is liberal on ETAS nulls, but $N_\mathrm{obs}=27$ exceeds
the ETAS null maximum (24), so the observed modern series count cannot
be attributed to local aftershock dynamics alone
...
$p_\mathrm{ETAS} \le 0.001$; FPR~=~1000/1000; $\bar{N}_\mathrm{ETAS}=15.4$.}
\label{fig:etas_null}
\end{figure}
...
null model.
The ETAS validation provides an independent test:
because $p_\mathrm{ETAS} \le 0.001$ with $N_\mathrm{obs}=27$ exceeding
the ETAS null maximum (24) across 1000 replicates (seed~=~42),
the observed multi-regional series count cannot be explained by standard
aftershock branching alone within 500~km.
...
for $\ge 1$ spurious series (mean $15.4$; max~$24$); $N_\mathrm{obs}=27$
exceeds the ETAS null maximum. Previously reported FPR~=~0/100 was an API bug
(\texttt{min\_regions} TypeError). Rejecting the ETAS null does not prove
a physical triggering mechanism.
...
Formal ETAS validation has been completed ($p_\mathrm{ETAS} \le 0.001$;
