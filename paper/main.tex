...
\textbf{Secondary diagnostic:} catalog-matched WLS calibration finds mean
\textbf{27.0} false series, matching $N_\mathrm{obs}=27$ ($p_\mathrm{ETAS}=1.0$) ---
detector--calibration coupling; \textbf{not adequate} as sole falsification test
($K$ likely inflated by 24-event WLS).
...
\begin{keypoints}
  ...
  \item \textbf{Secondary diagnostic} (catalog WLS): mean $= 27.0$,
        $p_\mathrm{ETAS}=1.0$ --- detector--calibration coupling; not sole
        falsification ($K$ inflated by 24-event WLS).
...
\textbf{Catalog-calibrated WLS} (diagnostic only):
...
Secondary & Catalog WLS & 0.103 & 0.495 & 27.0 & 1.0 \\
\hline
\end{tabular}
\end{table}
...
decoupled from detector). \textbf{Secondary null} diagnoses
detector--calibration coupling; $p_\mathrm{ETAS}=1.0$ is \textbf{not} adequate
as the sole falsification test because $K \approx 0.495$ is likely inflated
by simplified WLS on 24 GK aftershocks.
...
\textbf{Secondary diagnostic:} catalog-calibrated WLS ($\mu \approx 0.103$,
$K \approx 0.495$) yields $p_\mathrm{ETAS}=1.0$ (mean $=27.0$) --- a
detector--calibration coupling; \textbf{not adequate} as the sole falsification
test because $K$ is likely inflated by simplified WLS on 24 GK aftershocks.
...
Catalog WLS (secondary) & $= 1.0$ & Matches (mean $= 27.0$) &
  Detector--calibration coupling; \textbf{not} sole falsification \\
...
\textbf{Secondary diagnostic} catalog-calibrated validation ($n=1000$) yields
$p_\mathrm{ETAS}=1.0$ and mean false series $= 27.0$, matching
$N_\mathrm{obs}=27$ --- detector--calibration coupling, not independent proof.
...
\begin{enumerate}
  ...
  \item \textbf{Secondary diagnostic} (catalog WLS): mean $= 27.0$,
        $p_\mathrm{ETAS}=1.0$ --- detector--calibration coupling; liberal
        detector; no physical mechanism supports global-series claims.
