...
\textbf{Objective.} Test whether physically meaningful multi-regional
global series exist in the $M \ge 6.5$ catalog (primary window 1973--2026).

\textbf{Scope.} We analyze \textbf{detector candidates} with spatial gates
(mean GC $> 1500$~km) but \textbf{validate only temporal excess} vs.\
catalog-calibrated ETAS; the spatial linkage hypothesis is \textbf{not tested}.
We extend prior global rate tests with a complementary
Baiesi--Paczuski $\eta$ detector and catalog-calibrated temporal ETAS null
(\S\ref{subsec:etas-calibration}).
Conclusions are limited to temporal clustering in detector windows;
spatial linkage requires future spatial ETAS \citep{Ogata1998}.
...
\hline
(a) Permutation & Independent event times (fixed coordinates) & Temporal clustering & mean $\log_{10}\eta_\mathrm{NN}$; $n=10{,}000$ & see \S\ref{subsec:primary-counts} \\
...
(c) Global-series hypothesis & No teleseismic multi-regional chains beyond local clustering & Such chains exist & --- & \textbf{Not tested by temporal-only ETAS}; spatial null open \\
...
(all epochs).
Permutation: $p = 0.0001$ (1/10{,}001), $z = -6.17$.
...
Permutation vs.\ ETAS test different hypotheses (Poisson times vs.\ calibrated
temporal null) --- \S\ref{sec:dual-etas-interpret}.
