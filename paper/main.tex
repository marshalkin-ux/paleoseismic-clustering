...
Qualitatively, a diagnostic tectonic-vs-Euclidean comparison
...
on random $M \ge 6.5$ pairs (1973+, $n = 249$) yields median
$\Delta\log_{10}\eta = +0.28$ ($1.6[\log_{10} r_\mathrm{tect} - \log_{10} r_\mathrm{eucl}]$);
${\sim}98\%$ of sampled pairs use the 1.5$\times$ GC fallback
($1.6 \log_{10} 1.5 \approx 0.28$).
...
\begin{enumerate}
  ...
  \item Tectonic-path distance along \citet{Bird2003} plate boundaries is
        implemented with a documented diagnostic comparison to great-circle
        distance (median $\Delta\log_{10}\eta = +0.28$ on $n = 249$ random
        pairs; \texttt{scripts/generate\_grl\_figures.py}); formal parameter
