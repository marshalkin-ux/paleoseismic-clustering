...
def build(s):
    ...
    story.append(P(
        ...
        "full numbers \u2014 Sec. 4.1). Title refers to <b>detection geometry</b>; "
        "<b>inferential tests are temporal-only</b>. Spatial component not modeled. "
        "Pre-1900 NOAA \u2014 Supplementary S3.",
        s["abstract"]
    ))
    story.append(P(
    ...
    story.append(P(
        "Michael [2011] и Shearer &amp; Stark [2012] не нашли аномальной глобальной "
        "кластеризации M\u22657, но ни одно исследование не тестировало мультирегиональную "
        "\u03b7-связность при M\u22656.5 с catalog-calibrated ETAS.",
        s["body"]
    ))
    story.append(P(
        "<b>Двухфазный дизайн.</b> (1) <b>Детекция</b> \u2014 пространственно-временной "
        "<i>поиск кандидатов</i> (\u03b7 + ворота GC&gt;1500\u00a0км). "
        "(2) <b>Валидация</b> \u2014 <i>только temporal</i> ETAS MLE + hold-out; "
        "не тестирует spatial linkage (Ogata 1998 \u2014 future work).",
        s["body"]
    ))
    story.append(P(
    ...
    story.append(P(
        "<b>\u00a71.1.</b> (a) Permutation (диагностика): пуассоновские времена отвергнуты "
        "\u2014 ожидаемо (Methods). (b) ETAS MLE \u2192 \u00a74.1. "
        "(c) Глобальные серии: <b>не тестируется (spatial ETAS open)</b>. "
        "(d) WLS (S2): coupling illustration.",
        s["body_ni"]
    ))
    ...
    story.append(P(
        "Bird (2003) изучался в раннем конвейере; <b>исключён из первичного анализа</b>; "
        "синтетический benchmark (ROC/precision-recall) в этой работе <b>не</b> проводился "
        "(см. <code>paper/supplementary.md</code> \u00a7S1). Первичный анализ \u2014 "
        "только great-circle.",
        s["body"]
    ))
    ...
    story.append(P(
        "Полный анализ выдаёт <b>47 кандидатов детектора</b> (не «открытые серии»): "
        "27\u00a0современных, 15\u00a0ранних (ограничение quality_score), "
        "5\u00a0исторических (не значимы, p=0,46). "
        "142 кластера-кандидата до фильтрации. Первичная валидация \u2014 таблица ETAS ниже.",
        s["body"]
    ))
