...
def build(s):
    ...
    story.append(Paragraph(
        ...
        "Итоговый каталог: <b>4418 событий</b> M\u22656.5 (2150\u00a0г.\u00a0до\u00a0н.\u044d.\u20132026); "
        "из них 2041\u00a0\u2014 современный период (1973\u20132026), "
        "2179\u00a0\u2014 ранний инструментальный (1900\u20131972), "
        "198\u00a0\u2014 исторические (до\u00a01900).",
        s["body"]
    ))
    ...
    story.append(Paragraph(
        ...
        "Дейкстры (пакет NetworkX). Для геодинамически слабосвязанных систем "
        "(r\u209c\u2091\u2090\u209c > r\u2047\u2091 \u00d71.5) применяется штрафной "
        "коэффициент: r\u1d62\u2c7c = 1.5 \u00d7 r\u2047\u2091.",
        s["body"]
    ))
    ...
    story.append(Paragraph(
        "Адаптация из Baiesi & Paczuski (2004), \u0434\u0444=1.6 (фрактальная размерность).",
        s["caption"]
    ))
    ...
    story += SSSEC("2.3 Алгоритм поиска серий", s)
    steps = [
        ("1.", "Декластеризация Gardner\u2013Knopoff [1974] для удаления локальных афтершоков."),
        ("2.", "Построение леса связей: для каждого j\u00a0\u2014 поиск родителя i* = argmin \u03b7\u1d62\u2c7c при \u03b7\u1d62\u2c7c < \u03b7\u2080."),
        ("3.", "Критерий глобальной серии: N\u22654 события; M\u22656.5; \u22653 зон Флинна\u2013Энгдала."),
        ("4.", "Скользящие окна: 1, 2, 5\u00a0лет с шагом 1\u00a0год; перекрывающиеся группы объединяются."),
    ]
    ...
    story.append(Paragraph(
        "Полный исторический анализ выявил <b>47 глобальных серий</b> в трёх эпохах: "
        "27\u00a0современных (1973\u20132026, p&lt;0.0001), 15\u00a0ранних "
        "(1900\u20131972, p=0.007), 5\u00a0исторических (p=0.46). "
        "Алгоритм также находит 142 кластера-кандидата до фильтрации по окнам. "
        "Топ-5 серий представлены в таблице ниже.",
        s["body"]
    ))
    ...
    story.append(Paragraph(
        "Таблица\u00a01. Топ-5 мультирегиональных сейсмических серий (\u22653 зон Флинна\u2013Энгдала).",
        s["caption"]
    ))
    story.append(Spacer(1, 0.2 * cm))
    ...
    story.append(Paragraph(
        "<b>Перспективы:</b> расчёт кулоновских напряжений (\u0394CFS) для серий "
        "S047, S170, S095; учёт механизмов очага; расширение ETAS-модели "
        "с явными дальними связями; публикация кода (GitHub/Zenodo).",
        s["body_ni"]
    ))
    ...
    refs = [
        ...
        "18. Young\u00a0J.B. et\u00a0al. The Flinn\u2013Engdahl regionalization scheme: the 1995 revision // Phys. Earth Planet. Int. 1996. Vol.\u00a096. P.\u00a0223\u2013297. DOI: 10.1016/0031-9201(96)03141-X",
        "19. Storchak\u00a0D.A. et\u00a0al. Public release of the ISC-GEM Global Instrumental Earthquake Catalogue // Seismol. Res. Lett. 2013. Vol.\u00a084. P.\u00a0810\u2013815. DOI: 10.1785/0220130034",
        "20. Kagan\u00a0Y.Y. Fractal dimension of brittle fracture // J.\u00a0Nonlinear Sci. 1991. Vol.\u00a01. P.\u00a01\u201316. DOI: 10.1007/BF01209146",
    ]
    for r in refs:
