...
def build(s):
    ...
    story.append(Paragraph(
        ...
        "Обнаружение серий\u00a0\u2014 статистическая аномалия (\u03b7-связи, p-values); "
    ...
    story.append(Paragraph(
        ...
        "<b>Ограничения:</b> привязка 500\u00a0км и множитель 1.5\u00d7\u00a0GC \u2014 "
    ...
    story.append(Paragraph(
        "b=1.0 \u2014 конвенция Baiesi &amp; Paczuski (2004) для сопоставимости \u03b7; "
    ...
    story.append(Paragraph(
        "Сырые каталоги \u2192 дедупликация \u2192 GK-декластеризация (главные толчки) "
        "\u2192 \u03b7 NN-лес \u2192 скользящие окна \u2192 критерии серии "
        "\u2192 MC/ETAS/FDR. GK: 24 афтершока (2017/2041, 98.8\u00a0%). "
        "ZBZ: 1 зависимое (2040/2041, 100\u00a0%) \u2014 проверка чувствительности, "
        "не последовательный фильтр.",
        s["body"]
    ))

    story += SSSEC("2.4 Порог \u03b7\u2080 и алгоритм поиска серий", s)
    story.append(Paragraph(
    ...
    story.append(Paragraph(
        ...
        "порог 500\u00a0км. 100 синтетических каталогов (seed=42); "
        "FPR=0/100; p\u2091\u209c\u2090\u209b=0.0000 (дискретный тест, 1/101). "
        "<b>Ограничение:</b> FPR=0/100 при seed=42; мультiseed-робастность \u2014 "
        "будущая работа; не доказывает механизм. "
