...
def build(s):
    ...
    story.append(Paragraph(
        ...
        "4418\u00a0записей CSV, 4267\u00a0событий M\u22656.5 (2150\u00a0г.\u00a0до\u00a0н.\u044d.\u20132026\u00a0гг.; "
        ...
        "S170 (46\u00a0событий, 12\u00a0регионов, M\u2098\u2090\u2093=9.1, 2002\u20132023).",
        s["abstract"]
    ))
    ...
    story.append(Paragraph(
        ...
        "M\u2098\u2090\u2093=8.8); the most spatially extensive modern series is S170 "
        "(46 events, 12 Flinn\u2013Engdahl regions, 2002\u20132023, M\u2098\u2090\u2093=9.1), "
    ...
    story.append(Paragraph(
        ...
        "4267\u00a0уникальных событий (из 4418\u00a0сырых записей). "
        "USGS\u00a0ComCat содержит <b>2088</b> сырых событий M\u22656.5 за 1973\u20132026; "
        "после объединения и дедупликации \u2014 <b>2041</b> событие M\u22656.5 "
        "(без фильтрации по quality_score). GK-декластеризация удаляет ~24\u00a0афтершока "
    ...
    story.append(Paragraph(
        ...
        "только m<sub>i</sub> родителя. b=1.0 соответствует оригинальной постановке "
        "Baiesi &amp; Paczuski (2004); эмпирическое b-значение каталога 0.911\u00b10.018 "
        "(используется в Monte Carlo-тесте). \u03b7 \u2014 относительная мера; "
    ...
    story.append(Paragraph(
        ...
        "<b>ETAS-валидация</b> (<code>ETASCatalogGenerator</code>, "
        "<code>run_etas_validation.py</code>). Параметры: "
        "\u03bc=0.008, K=0.08, \u03b1=1.0, c=0.005\u00a0сут., p=1.1; "
        "max_trigger_distance_km=500\u00a0км (связи &gt;500\u00a0км запрещены). "
        "Параметр c=0.005\u00a0сут. выбран консервативно для ускорения затухания "
        "афтершоков в нулевой модели; даже при таком строгом условии FPR=0/100. "
        "100 синтетических каталогов (ветвящийся пуассоновский процесс, seed=42); "
        "на каждом \u2014 тот же алгоритм global_series (N\u22654, \u22653 FE, окно 2\u00a0г.). "
        "FPR = доля каталогов с \u22651 ложной серией = 0/100; "
        "p\u2091\u209c\u2090\u209b = P(N\u2091\u209c\u2090\u209b \u2265 N\u2090\u2099\u209b) = 0.0000 "
        "(дискретный тест, разрешение 1/101). "
        "<b>Коррекция FDR.</b> Бенджамини\u2013Хохберга [1995] при q=0.05: "
        "45 из 47 серий значимы. "
    ...
    story.append(Paragraph(
        ...
        "они тестировали <i>глобальные частоты</i> и <i>тренды event rates</i> M\u22657, "
        "а не пространственно-временную <i>структуру связей</i> между регионами. "
        "Наш \u03b7-метрик выявляет паттерны мультирегиональной кластеризации независимо "
        "от секулярных изменений частоты; выводы дополняют друг друга. "
        "<b>Ранний инструментальный период (1900\u20131972):</b> 15\u00a0серий значимы "
        "при p=0.007 (z=\u22122.43), что усиливает вывод о кластеризации вне современного "
        "окна; однако quality_score&lt;0.7 до 1960\u00a0г. требует осторожной интерпретации. "
        "<b>Ограничения:</b> исторический период p=0.46, только 47\u00a0событий M\u22656.5 "
        "до 1900\u00a0г.; p\u2091\u209c\u2090\u209b=0.0000 \u2014 дискретный тест на 100 "
        "каталогах (не доказывает механизм); \u03b7-метрика корреляционная, "
        "глубина и механизмы очага не используются.",
        s["body"]
    ))
    story.append(PageBreak())
    ...
    conclusions = [
        ...
        ("2.", "ETAS-валидация (\u03bc=0.008, K=0.08, \u03b1=1.0, c=0.005\u00a0сут., "
               "p=1.1, порог 500\u00a0км; FPR=0/100) и FDR (45/47\u00a0при q=0.05) "
               "подтверждают неслучайность серий."),
        ...
        ("5.", "<b>Две интерпретационные ветви:</b> (a)\u00a0если серии реальны \u2014 "
               "последствия для опасности (условная вероятность в удалённых зонах, "
               "дальние ядра ETAS); (b)\u00a0если артефакты \u2014 FDR+ETAS конвейер "
               "остаётся воспроизводимым null-test для будущих каталогов."),
    ]
